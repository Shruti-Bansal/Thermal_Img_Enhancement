...
import copy
import wandb
from Datasets.MS2_utils import visualize_disp_as_numpy
...
def save_images(logger, mode_tag, images_dict, global_step):
    ...
    for tag, values in images_dict.items():
        ...
        for idx, value in enumerate(values):
            ...
            logger.add_image(
                image_name, vutils.make_grid(value, padding=0, nrow=1, normalize=True, scale_each=True), global_step
            )


def wandb_scalars(mode_tag, scalar_outputs, global_step):
    scalar_dict = {f"{mode_tag}/{key}": value for key, value in scalar_outputs.items()}
    scalar_dict = {k: v[0] if isinstance(v, list) and len(v) > 0 else v for k, v in scalar_dict.items()}
    scalar_dict[f"{mode_tag}/step"] = global_step
    wandb.log(scalar_dict)


def wandb_images(mode_tag, images_dict, global_step):
    LOG_LIST = [
        "train/errormap",
        "test/imgL",
        "test/disp_est",
    ...
    for tag, values in images_dict.items():
        ...
        for idx, value in enumerate(values):
            ...
            value = value[:1]
            value = torch.from_numpy(value)
            img_name = f"{mode_tag}/{tag}"
            if len(values) > 1:
                img_name = img_name + "_" + str(idx)
            if img_name not in LOG_LIST:
                continue
            # if tag == 'disp_est' or tag == 'disp_gt':
            #     disp = value.squeeze(0).squeeze(0)
            #     value = visualize_disp_as_numpy(disp)
            logDict = {img_name: wandb.Image(value), "step": global_step}
            wandb.log(logDict)
